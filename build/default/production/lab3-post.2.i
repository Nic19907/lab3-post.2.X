...
; CONFIG1
  ...
  CONFIG LVP = ON ; Low Voltage Programming Enable bit (((PORTB) and 07Fh), 3 pin has digital I/O, HV on MCLR must be used for programming)
...
;************************** Reloj de 100ms y 1s ********************************
clock_100ms:
    btfss ((INTCON) and 07Fh), 2 ;salta si la bandera del overflow esta activa
    goto $-1
...
;************************** 7 segmentos ****************************************
config_contador:
    btfsc ((PORTD) and 07Fh), 0 ;Push button en Pull DOWN
    call inc_contador
    btfsc ((PORTD) and 07Fh), 1 ;lo mismo que ((PORTD) and 07Fh), 0
    call dec_contador
...
inc_contador:
    ...
    btfsc ((PORTD) and 07Fh), 0 ;(el boton esta ciendo precionado) hasta que no se
